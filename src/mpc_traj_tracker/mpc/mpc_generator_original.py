...
def cost_refstate_deviation(state:CasadiState, ref_state:CasadiState, weights:cs.SX):
...
class MpcModule:
    ...
    def build(self, dynamics:Callable[[SamplingTime, CasadiState, CasadiAction], CasadiState], use_tcp:bool=False):
        ...
        Conmment:
            ...
            states (s): x, y, theta, e (e is the channel width / allowable divation from reference)
        ...
        u = cs.SX.sym('u', self.nu*self.N_hor)    # 0. Inputs at every predictive step
        s = cs.SX.sym('s', 2*self.ns + self.nu)   # 1. Current and goal states, initial inputs
        q = cs.SX.sym('q', self.config.nq)                      # 2. Penalty parameters
        r = cs.SX.sym('r', self.ns*self.N_hor+ self.N_hor)     # 3. Reference path + speed reference in each step
        c = cs.SX.sym('c', self.ns)
        ...
        q_stc = cs.SX.sym('qstc', self.N_hor)            # 7. Static obstacle weights
        q_dyn = cs.SX.sym('qdyn', self.N_hor)            # 8. Dynamic obstacle weights
