...
pub const NAVI_TEST_NUM_DECISION_VARIABLES: usize = 40;

/// Number of parameters
pub const NAVI_TEST_NUM_PARAMETERS: usize = 2658;

/// Number of parameters associated with augmented Lagrangian
pub const NAVI_TEST_N1: usize = 40;

/// Number of penalty constraints
pub const NAVI_TEST_N2: usize = 15;
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(NAVI_TEST_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, NAVI_TEST_N1, NAVI_TEST_N2)
}
...
pub fn solve(
    ...
    assert_eq!(p.len(), NAVI_TEST_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), NAVI_TEST_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_navi_test::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_navi_test::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f1 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_navi_test::mapping_f1(u, p, res);
        Ok(())
    };
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_navi_test::mapping_f2(u, p, res);
        Ok(())
